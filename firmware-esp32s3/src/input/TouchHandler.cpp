...
bool TouchHandler::begin() {
  ...
  if (!initialized_) {
    Serial0.println("[touch] controller not detected at 0x3B");
  } else {
    Serial0.println("[touch] initialized (AXS15231B)");
  }
...
bool TouchHandler::poll(TouchEvent &event) {
  ...
  if (now < backoffUntilMs_) return false;
  if (now - lastPollMs_ < kPollIntervalMs) return false;
  lastPollMs_ = now;
  ...
  if (!readTouchPacket(data, sizeof(data))) {
    ...
    if (++consecutiveReadFailures_ >= 5) {
      initialized_ = false;
      Serial0.println("[touch] read failed repeatedly, disabling polling");
    }
    return false;
  }
  consecutiveReadFailures_ = 0;
  ...
  if (points == 0 || points >= 5) {
    if (touchActive_) {
      ++emptyTouchSamples_;
      if (emptyTouchSamples_ < kReleaseConfirmSamples) return false;
      touchActive_       = false;
      emptyTouchSamples_ = 0;
      event.touched      = false;
      event.x            = lastX_;
  ...
  const uint16_t rawLongAxis  =
      static_cast<uint16_t>(((data[2] & 0x0F) << 8) | data[3]);
  const uint16_t rawShortAxis =
